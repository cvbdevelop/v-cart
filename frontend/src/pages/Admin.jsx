...
import { Package, Trash2, Plus, Save, Edit, X, UploadCloud, Loader2, LogOut, ClipboardList, CheckCircle } from 'lucide-react';

function Admin() {
  const navigate = useNavigate();
  
  // State សម្រាប់គ្រប់គ្រង Tab ថាតើកំពុងមើល "products" ឬ "orders"
  ...
  useEffect(() => {
    ...
    fetchProducts();
    fetchOrders(); // ទាញយកបញ្ជាទិញពេលបើកទំព័រ
  }, [navigate]);
  ...
  const uploadImageToServer = async (file) => {
    ...
    try {
      ...
      console.error(err); alert('មានបញ្ហាក្នុងការ Upload រូបភាព'); return null;
    }
  };
  ...
  const handleEditClick = (product) => {
    ...
    setFormData({
      name: product.name || '', price: product.price || '', stock: product.stock || '', category: product.category || 'general',
      image: product.image || '', description: product.description || '', images: product.images || [],
      colors: product.colors ? product.colors.join(', ') : '', sizes: product.sizes ? product.sizes.join(', ') : '', storage: product.storage ? product.storage.join(', ') : ''
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(url, {
        method: method, headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify(formattedData)
      });
      const data = await res.json();
      if (data.success || res.ok) {
        alert(editingId ? 'បានកែប្រែទំនិញជោគជ័យ!' : 'បានបន្ថែមទំនិញជោគជ័យ!');
        handleCancelEdit(); fetchProducts();
      } else { alert(data.message || 'ប្រតិបត្តិការបរាជ័យ'); }
  ...
  const handleUpdateOrderStatus = async (orderId, newStatus) => {
    try {
      const res = await fetch(`https://v-cart-backend.onrender.com/api/orders/${orderId}/status`, {
        method: 'PUT',
    ...
    } catch (err) { console.error(err); alert('មានបញ្ហាក្នុងការកែប្រែស្ថានភាព'); }
  };

  const handleDeleteOrder = async (orderId) => {
    if (!window.confirm('តើអ្នកពិតជាចង់លុបការបញ្ជាទិញនេះមែនទេ?')) return;
    const token = localStorage.getItem('adminToken');
    try {
      const res = await fetch(`https://v-cart-backend.onrender.com/api/orders/${orderId}`, {
        method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-10">
      <div className="container mx-auto px-4 max-w-[1200px]">
        ...
        {/* Tabs សម្រាប់ឆ្លាស់គ្នា */}
        <div className="flex gap-4 mb-8 border-b border-gray-200 pb-4">
          <button 
            onClick={() => setActiveTab('products')}
            className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold transition ${activeTab === 'products' ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
          >
            <Package size={20} /> គ្រប់គ្រងទំនិញ
          </button>
          <button 
            onClick={() => setActiveTab('orders')}
            className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold transition ${activeTab === 'orders' ? 'bg-orange-500 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100'}`}
          >
            <ClipboardList size={20} /> ការបញ្ជាទិញ {orders.filter(o => o.status === 'Pending').length > 0 && <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{orders.filter(o => o.status === 'Pending').length} ថ្មី</span>}
          </button>
        </div>

        {/* ================= ផ្ទាំងគ្រប់គ្រងទំនិញ (Products Tab) ================= */}
        {activeTab === 'products' && (
        ...
        {/* ================= ផ្ទាំងការបញ្ជាទិញ (Orders Tab) ================= */}
        {activeTab === 'orders' && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
              <h2 className="text-lg font-bold text-gray-800">បញ្ជីការបញ្ជាទិញសរុប ({orders.length})</h2>
            </div>
            
            <div className="overflow-x-auto p-4">
              {loadingOrders ? (
                <div className="text-center py-10 text-gray-500">កំពុងទាញយកការបញ្ជាទិញ...</div>
              ) : orders.length === 0 ? (
                <div className="text-center py-10 text-gray-500">មិនទាន់មានការបញ្ជាទិញនៅឡើយទេ</div>
              ) : (
                <table className="w-full text-left text-sm text-gray-600">
                  <thead className="bg-gray-100 text-gray-800 font-bold">
                    <tr>
                      <th className="p-3 rounded-l-lg">ថ្ងៃខែឆ្នាំ</th>
                      <th className="p-3">អតិថិជន & ទំនាក់ទំនង</th>
                      <th className="p-3">អាសយដ្ឋាន</th>
                      <th className="p-3">ការកុម្ម៉ង់ (ទំនិញ)</th>
                      <th className="p-3">សរុបទឹកប្រាក់</th>
                  ...
                  <tbody>
                    {orders.map(order => (
                      <tr key={order._id} className="border-b border-gray-50 hover:bg-gray-50 transition align-top">
                        <td className="p-3">
                        ...
                        <td className="p-3 max-w-[200px] text-xs leading-relaxed">{order.address}</td>
                        <td className="p-3 max-w-[250px]">
                          <ul className="list-disc pl-4 text-xs space-y-1">
                            {order.items.map((item, idx) => (
                              <li key={idx}>
                                {item.name} <span className="text-orange-500 font-bold">x{item.quantity || 1}</span>
                          ...
                          <select 
                            ...
                            className={`px-2 py-1.5 rounded-lg text-xs font-bold border outline-none cursor-pointer
                              ${order.status === 'Pending' ? 'bg-red-50 text-red-600 border-red-200' : 
                                order.status === 'Processing' ? 'bg-orange-50 text-orange-600 border-orange-200' : 
                        ...
                        <td className="p-3 flex justify-center">
                          <button onClick={() => handleDeleteOrder(order._id)} className="p-2 text-red-500 hover:bg-red-50 rounded-md transition" title="លុបចោល">
                            <Trash2 size={18} />
                          </button>
                        </td>
                      </tr>
